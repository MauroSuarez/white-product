...
export function useHeaderItems({
    ...
    const boothItem: Array<HeaderItemProps<TItems>> = [
      {
        ...
        format: ({ label, path }) => (
          <DropdownMenuItem
            onClick={() => handleNavigate && handleNavigate(path)}
          >
            {label}
          </DropdownMenuItem>
        )
          ...
          <DropdownMenuItem onClick={handleChangeTheme}>
            <div className="flex flex-wrap justify-between w-full items-center">
              ...
              <div className="text-foreground">
                <Icon name={theme === "light" ? "MoonIcon" : "SunIcon"} />
              </div>
            </div>
          </DropdownMenuItem>
        )
      ...
      ...separator,
      {
        ...
        visible: userRol !== "GUEST",
        format: ({ label }) => (
          <DropdownMenuItem onClick={handleSignOut}>{label}</DropdownMenuItem>
        )
      }
    ];
    const arrMenuAdmin: Array<HeaderItemProps<TItems>> = [...boothItem];
    ...
    const arrMenuGuest: Array<HeaderItemProps<TItems>> = [
      {
        ...
        format: ({ label, path }) => (
          <DropdownMenuItem
            onClick={() =>
              handleAuthModal && handleAuthModal({ open: true, type: "signin" })
            }
          >
            {label}
          </DropdownMenuItem>
      ...
      },
      {
        ...
        format: ({ label, path }) => (
          <DropdownMenuItem
            onClick={() =>
              handleAuthModal && handleAuthModal({ open: true, type: "signup" })
            }
          >
            {label}
          </DropdownMenuItem>
  ...
  const headersType: { [key in THeaderType]: Array<HeaderItemProps<TItems>> } =
    useMemo(() => {
      const modePanelAdmin: Array<HeaderItemProps<TItems>> = [
        {
          ...
          format: ({ label, path }) => (
            <Button
              onClick={() => handleNavigate && handleNavigate(path)}
              variant="outline"
              className="relative hidden md:flex h-10 min-w-[100px]"
            >
              {label}
      ...
      return {
        empty: userRol !== "GUEST" ? [...dropDownMenu] : [],
        default: [
          {
            id: "about-us",
            label: "Acerca de nosotros",
          ...
          ...goScan,
          {
            ...
            format: ({ label, path }) => (
              ...
              >
                ...
                <Icon
                  name="BackpackIcon"
                  className="ml-3 h-5 w-5 text-foreground"
                />
                <Dot />
              </Button>
            ),
          ...
          },
          {
            ...
            format: ({ label, path }) => (
              ...
              >
                ...
                <Icon
                  name="PlusIcon"
                  className="h-6 w-6 ml-2 text-background"
                />
              </Button>
            ),
            visible: true
        ...
        detail: [...goScan, ...modePanelAdmin, ...dropDownMenu],
        workshop: [
          {
            id: "now",
            label: "Hoy",
          ...
          },
          {
            ...
            format: ({ label }) => (
              <Button
                variant="ghost"
                className="hidden md:flex h-10 min-w-[100px] relative"
              >
                {label}
                <Dot />
              </Button>
