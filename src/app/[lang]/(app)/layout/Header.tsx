...
import { useRouter } from "next/navigation"; // Changed from direct router import
import { usePositionScroll } from "@/presentation/hooks/usePositionScroll";
import type { TUserRol } from "@/core/domain/entities/UserRol";
...
export function Header<T>({
  ...
  const router = useRouter(); // Use the router hook instead of direct import
  ...
  return (
    ...
    >
      <div className="mx-10 items-center flex w-full">
        ...
        <div
          className={`flex justify-end items-center ${containerButtons} space-x-2 sm:space-x-4 border border-black`}
        >
          {isLoading &&
              ...
              return item.visible ? (
                <React.Fragment key={`item-header-${i}`}>
                  {typeof item.format === "function"
                    ? item.format({ label: item?.label, path: item?.path })
                    : item.label}
                </React.Fragment>
              ) : null;
            })}
