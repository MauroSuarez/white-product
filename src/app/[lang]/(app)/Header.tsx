...
const Header: React.FC<HeaderProps> = ({}) => {
  ...
  const handleItemClick = (item: MenuItem) => {
    ...
    if (item.path) router.push(`/es/${item.path}`);
  ...
  return (
    <>
      <header
        className={`border-b border-grey-50 transition-all duration-300 ${
          isSmall ? "h-16" : "h-28"
        } flex items-center`}
      >
        <div className="w-full px-10 mx-auto flex justify-between">
          <Link href={"/es"} className="flex items-center">
            <div className="flex items-center flex-nowrap">
              ...
              <Typography className="w-full flex text-primary ml-2">
                FreeWheels
              </Typography>
            </div>
          </Link>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center border rounded-full shadow-sm overflow-hidden min-w-[450px] w-auto">
              <Input
                ...
                className={`flex ${
                  isSmall ? "h-10" : "h-12"
                } items-center px-4 py-4 border-none focus:ring-0 focus:outline-none`}
              />
              <button
                className={`text-white ${
                  isSmall ? "px-2 py-2" : "px-6 py-2"
                } mr-1 transition-all duration-300 rounded-full bg-primary`}
              >
                <Icon
              ...
              <div className="rounded-full px-4 border border-gray-300 items-center h-12 flex justify-center cursor-pointer">
                <div className="flex justify-center space-x-2 items-center">
                  ...
                  <Avatar className="h-10 w-10 bg-gray-300 rounded-full flex items-center justify-center">
                    <AvatarImage
                      src="https://github.com/shadcn.pngd"
                      alt="@shadcn"
                    />
                    <AvatarFallback>
